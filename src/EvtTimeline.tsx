...
const EvtTimeline = ({ events }: { events: any }) => {
  const hostMap = Object.fromEntries(hosts);

  const timelineEvents = events ? (
    events.map((event: any) => {
      const sourceStr = hostMap[event.source_ip] || event.source_ip;
      const destStr = hostMap[event.destination] || event.destination;
